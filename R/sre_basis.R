...
#' estimates.
#' 
#' @param object A smooth specification object, usually generated by a 
...
#' @param knots A list containing any knots supplied for setting up the smooth over
#'   the \code{"by"} variable.
#' 
#' @details To specify a basis for the smooth over the \code{"by"} variable,
...
smooth.construct.sre.smooth.spec <- function(object, data, knots) {
  ...
  object.s = object.re <- object
  
  # s smooth
  object.s <- object
  object.s$term <- object$by
  object.s$by <- "NA"
  ...
  object$X <- do.call("cbind", rep(list(smooth.s$X), nre+1))
  object$S <- list(as.matrix(bdiag(smooth.s$S[[1]], diag(rep(0,N*smooth.s$bs.dim)))),
                   as.matrix(bdiag(diag(rep(0,smooth.s$bs.dim)),
                         smooth.re$S[[1]] %x% smooth.s$S[[1]])))
  object$bs.dim <- smooth.s$bs.dim * (nre+1)
  object$rank <- c(smooth.s$rank, smooth.s$rank*nre)
  object$null.space.dim <- object$bs.dim - sum(object$rank)
  class(object) <- "sre.smooth"
  object
}

#' Predict.matrix method
#' 
Predict.matrix.sre.smooth <- function(object, data) {
